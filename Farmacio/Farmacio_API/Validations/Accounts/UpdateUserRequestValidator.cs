...
namespace Farmacio_API.Validations.Accounts
{
    public class UpdateUserRequestValidator<T> : AbstractValidator<T> where T : UpdateUserRequest
    {
        public UpdateUserRequestValidator()
        {
            ...
            RuleFor(request => request.PID).NotNull().Length(13).Must(pid => !string.IsNullOrEmpty(pid) && pid.All(charInPid => '0' <= charInPid && charInPid <= '9')).WithMessage("Pid must be 13 digits.");
